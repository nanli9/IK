...
Quaternion::Quaternion(RigidTransform4d m)
{
	...
	Quaternion x = Quaternion(cos(alpha / 2), sin(alpha / 2), 0, 0);
	Quaternion y = Quaternion(cos(theta / 2), 0, sin(theta / 2), 0);
	Quaternion z = Quaternion(cos(beta / 2), 0 , 0, sin(beta / 2));

	Quaternion result = z * y * x;
	for (int i = 0; i < 4; i++)
...
Quaternion Quaternion::QuaternionConjugate()
{
	Quaternion result;
...
double Quaternion::innerProduct(Quaternion q)
{
	double result;
